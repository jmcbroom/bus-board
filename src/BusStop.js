...
const BusStop = ({ stop }) => {
  ...
  useEffect(() => {
    const smartUrls = stop.properties.smart.map(
      (s) => `/.netlify/functions/stop?stopId=${s}&provider=smart`
    );
    const ddotUrls = stop.properties.ddot.map(
      (s) => `/.netlify/functions/stop?stopId=${s}&provider=ddot`
    );
    Promise.all(smartUrls.concat(ddotUrls).map((u) => fetch(u)))
      .then((responses) => Promise.all(responses.map((res) => res.json())))
      .then((predictions) => {
        let filtered = predictions.filter(
          (p) => Object.keys(p["bustime-response"]).indexOf("prd") > -1
        );
  ...
  let unexcluded = keys.filter(
    (r) => stop.properties.exclude.indexOf(parseInt(r.split("_")[0])) === -1 && r !== "Exclude"
  );
  return (
    <Panel>
      ...
      <div className="grid gap-2">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-sm text-gray-600 leading-tight">
                        {bus.routeDir.replace("bound", "")} to{" "}
                        {routes[rt][bus.routeDir.replace("bound", "")]}
                      </p>
                    </div>
                  </div>
                  <img
        ...
        {keys.length === 0 && (
          <div className="bg-gray-100">
            <p className="p-4 text-sm">No buses are currently predicted to arrive at this stop.</p>
          </div>
        )}
      </div>
    </Panel>
